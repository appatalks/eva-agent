...
work removes them without usage evidence and an explicit migration.
...
| Legacy model/settings values | `gpt-5-mini` is no longer a selector option but remains a live AIG backend (`openai:gpt-5-mini`) used for terminal planning, plus a direct GitHub Models mapping and token-budget entry; old cognition keys and the old theme value remain readable for saved-profile compatibility | Remove only with explicit key/value migration and contract coverage for existing profiles |
...
- Removal work must add a focused regression for existing profiles or sessions,
  update user documentation, and record the evidence in the tracking issue for
  the effort.
