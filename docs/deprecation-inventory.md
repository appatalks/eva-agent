...
| Legacy model/settings values | `gpt-5-mini` is no longer a top-level selector option but remains an accepted AIG backend for terminal planning; old cognition keys and the old theme value remain readable for saved-profile compatibility | Retain the compatibility value while all user-facing chat dispatch stays behind AIG |
