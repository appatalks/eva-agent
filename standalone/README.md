...
```sh
cd standalone/dist
chmod +x "Eva Standalone-5.5.7.AppImage"
"./Eva Standalone-5.5.7.AppImage" --eva-workspace-terminal-v1
```

If the host is missing FUSE (common on minimal containers and some distros), launch with extraction instead:

```sh
"./Eva Standalone-5.5.7.AppImage" --appimage-extract-and-run --eva-workspace-terminal-v1
```
