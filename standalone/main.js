...
function createWindow(acpBaseUrl) {
  ...
  mainWindow.webContents.on('context-menu', function(event, params) {
    ...
    const isExternalUrl = function(value) {
      try {
    ...
    if (isExternalUrl(params.linkURL)) {
      ...
      template.push({ label: 'Open Link in Browser', click: function() { shell.openExternal(params.linkURL); } });
      template.push({ label: 'Copy Link Address', click: function() { clipboard.writeText(params.linkURL); } });
    }
    if (params.mediaType === 'image') {
      if (template.length) template.push({ type: 'separator' });
      template.push({ label: 'Copy Image', click: function() { mainWindow.webContents.copyImageAt(params.x, params.y); } });
      if (isExternalUrl(params.srcURL)) {
        template.push({ label: 'Open Image in Browser', click: function() { shell.openExternal(params.srcURL); } });
      }
    }
