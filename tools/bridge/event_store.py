...
class EventRepositoryV2:
    ...
    def _transaction(self, connection=None):
        ...
        if own:
            ...
            savepoint = f"event_repo_{threading.get_ident()}_{time.time_ns()}"
        ...
        *,
        ...
        for attempt in range(3):
            try:
                ...
                # Only an independently durable matching event is idempotent.
                ...
                if duplicate:
                    existing = _row_to_dict(cur, duplicate)
    ...
    def events_since(self, cursor_sequence=0, limit=1000):
        ...
        cur = self._get_conn().execute(
            "SELECT * FROM MemoryEvents WHERE JournalSequence>? ORDER BY JournalSequence LIMIT ?",
    ...
    def has_legacy_receipt(self, event_id, projection_name, connection=None):
        ...
        conn = connection or self._get_conn()
        return conn.execute(
            "SELECT 1 FROM LegacyProjectionReceipts WHERE EventId=? AND ProjectionName=?",
    ...
    def record_legacy_receipt(self, event_id, projection_name, row_count=0, connection=None):
        ...
        conn = connection or self._get_conn()
        conn.execute(
            "INSERT OR IGNORE INTO LegacyProjectionReceipts (EventId,ProjectionName,RowCount) "
            "VALUES (?,?,?)",
    ...
    def pending_outbox(self, limit=50, destination=None):
        ...
        cur = conn.execute(
            ...
            "AND (o.NextAttemptAt IS NULL OR o.NextAttemptAt='' OR o.NextAttemptAt<=?)"
            + destination_sql + " ORDER BY o.CreatedAt,o.OutboxId LIMIT ?",
            tuple(params),
        )
        return [_row_to_dict(cur, row) for row in cur.fetchall()]
    ...
    def outbox_status(self):
        ...
        rows = self._get_conn().execute(
            "SELECT Destination,Status,COUNT(*) FROM MemoryOutbox GROUP BY Destination,Status"
        ).fetchall()
        result = {}
