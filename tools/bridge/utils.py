...
def _subagent_mcp_config(template, task):
    workspace_config = task.get("_workspace_mcp_config")
    if isinstance(workspace_config, dict):
        return copy.deepcopy(workspace_config)
    return copy.deepcopy(template.mcp_config)
...
def _classify_request_type(msg_lower):
    ...
    if re.search(r'\b(news|headlines?|breaking news|current events?|morning (?:briefing|report|update)|daily (?:briefing|report|update)|briefing)\b', m) or \
