...
def test_pr_automation_workflows():
    ...
    report("pr_automation_maintainer_summary", "Post maintainer summary" in readiness and "issues: write" in readiness and ".github/scripts/readiness_maintainer_comment.py" in readiness and "--has-marker readiness/pr-comments.json" in readiness and "optional maintainer comment could not be created" in readiness and "GITHUB_STEP_SUMMARY" in readiness and "github-actions[bot]" in open(".github/scripts/readiness_maintainer_comment.py").read() and "MAINTAINER_CATEGORY:" in readiness_agent and os.path.isfile(".github/scripts/readiness_maintainer_comment.py") and os.path.isfile("tools/tests/test_readiness_maintainer_comment.py"))
    report("pr_automation_maintainer_decision", "issue_comment:" in readiness_decision and "github.event.issue.pull_request" in readiness_decision and "python3 - <<'PY'" in readiness_decision and ".github/scripts/readiness_decision.py" not in readiness_decision and "actions/checkout" not in readiness_decision and "collaborators/$ACTOR/permission" in readiness_decision and "admin|maintain|write" in readiness_decision and "expected_head_sha" in readiness_decision and "PR Readiness / Terra verdict" in readiness_decision and "statuses: write" in readiness_decision and os.path.isfile(".github/scripts/readiness_decision.py") and os.path.isfile("tools/tests/test_readiness_decision.py"))
...
def test_coding_workspace_contract():
    ...
    report("coding_workspace_agent_auto_permission", 'permission_mode == "workspace_write"' in acp_client and 'tool_kind in {"read", "search", "fetch", "think"}' in acp_client and 'def _workspace_edit_target_is_local' in acp_client and 'decision="workspace-auto-allow-edit"' in acp_client and 'decision="workspace-auto-allow-execute"' in acp_client and 'def _workspace_sensitive_execute' in acp_client and '_WORKSPACE_SENSITIVE_COMMANDS' in acp_client and 'def _command_summary' in acp_client and 'permission_mode=permission_mode' in workspace_utils)
    ...
    harness_js = open("core/js/harness-control.js").read()
    report("coding_workspace_native_project_checks", "run_workspace_check" in harness_js and "smoke\\s*tests?" in harness_js and "runSelectedCheck" in workspace_ui and "injectWorkspaceStatusBubble" in options_js)
