...
def test_coding_workspace_contract():
    ...
    report("coding_workspace_preload_allowlist", all(value in standalone_preload for value in ("terminalCloseRoot", "workspaceListProjects", "workspaceSelectProject", "workspaceImportGitHub", "workspaceGitHubAuthStart", "workspaceGitHubAuthStatus", "workspaceSetMcpServer", "workspaceCreateRun", "workspaceCheckoutStatus", "workspaceRunAction")))
...
def test_agent_operations_contract():
    ...
    report("verbose_diagnostics_safe_toggle", 'id="verboseDiagnostics"' in html and "verbose_debug" in options and "_verbose_debug_emit" in bridge and "enabled_module_count" in bridge and '_verbose_debug_emit("permission_request"' in open("tools/bridge/acp_client.py").read() and "user_message" not in open("tools/bridge/telemetry.py").read().split("def _verbose_debug_emit", 1)[1].split("def _percentile", 1)[0])
