...
def test_coding_workspace_contract():
    ...
    report("coding_workspace_github_delivery", "requires_github_delivery" in bridge_core and "required_github_issue_state" in bridge_core and "This is a close-only request" in bridge_core and "authenticated `gh` CLI" in bridge_core and "if none exists, create a new issue" in bridge_core and "_workspace_github_delivery_url" in workspace_utils and "_verify_workspace_github_delivery(delivery_url, required_issue_state)" in workspace_utils and 'actual_state != expected_state' in workspace_utils and "GitHub Issues submission was required but not verified" in workspace_utils)
    ...
    report("coding_workspace_agent_auto_permission", 'permission_mode in {"workspace_write", "workspace_auto"}' in acp_client and 'permission_mode == "workspace_auto"' in acp_client and 'decision="workspace-auto-approve"' in acp_client and 'decision="workspace-auto-reject-boundary"' in acp_client and 'tool_kind in {"read", "search", "fetch", "think"}' in acp_client and 'def _workspace_edit_target_is_local' in acp_client and 'decision="workspace-auto-allow-edit"' in acp_client and 'decision="workspace-auto-allow-execute"' in acp_client and 'def _workspace_sensitive_execute' in acp_client and '_WORKSPACE_SENSITIVE_COMMANDS' in acp_client and 'def _command_summary' in acp_client and '"workspace_auto"' in workspace_utils and 'permission_mode=permission_mode' in workspace_utils and 'auto_approve' in workspaces)
