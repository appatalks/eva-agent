...
async function main() {
  ...
  assert.deepStrictEqual(remediationRequest, { repositoryName: 'appatalks/cs-proxy', objective: normalizedDependabotUrlRequest });
  assert.deepStrictEqual(nativeRemediationContext, { repositoryName: 'appatalks/cs-proxy', objective: normalizedDependabotUrlRequest });
  ...
  const cleanupRemovalRequest = 'Eva please cleanup and remove the appatalks/cs-proxy workspaces, as its no longer needed.';
  ...
  assert.strictEqual(cleanupRemovalRoute.projectName, 'appatalks/cs-proxy');
  const modelCleanupRemoval = await harness.execute({ action: 'remove_workspace', projectName: 'appatalks/cs-proxy' }, { source: 'model', userRequest: cleanupRemovalRequest });
