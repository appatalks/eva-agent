...
assert.match(source, /function isExplicitLocationMemoryRequest\(text\)/);
assert.match(source, /\/v1\/memory\/remember-location/);
assert.match(source, /Eva saved your location to durable memory/);
...
assert.match(options, /if \(!protectedNeedsDataRetrieval && window\.EvaHarness/);
